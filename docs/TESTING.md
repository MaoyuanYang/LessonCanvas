...
- [RESOLVED, 2026-09-03] F014 semantic retrieval suites: deterministic adapter contract in `apps/backend/tests/test_embedding.py` (fake hashed-ngram embedder — real weights never load in CI; fastembed is an `embedding` dependency group installed only into the deployed image, which pre-bakes the model weights per ADR-0007); write path, migration structure, deploy-time backfill idempotency (importlib-loaded `scripts/backfill_embeddings.py`), ranking/budget/exclusion/zero-relevance, per-family call-site retrieval with traced `retrieval.semantic_search` events, chunk-citation binding (cited ⊆ retrieved, payload citations stripped), injection inertness of retrieved text, F009 signature, deletion completeness, quota invariants, and API contract in `apps/backend/tests/test_retrieval.py`; web component states in `apps/web/__tests__/retrieval-surfaces.test.tsx` (chip expansion, ungrounded notice, sources chunk view, evidence summary chips); browser journey `apps/web/e2e/retrieval-journey.spec.ts` behind `E2E_RETRIEVAL=1` (deterministic stack, serial worker; note the uploaded source must not contain `课时分配` — the fake planner would skip gap questions, review.md SF-3). Deployment integrates the idempotent backfill as deploy-chain step 4/5 (`infra/scripts/deploy.sh`). The full owner-authorized live re-baseline under the new `retrieval_mode` signature (D5) executes at delivery with its own evidence files, never as a CI gate.
...
- [RESOLVED, 2026-09-03] F015 governed tool-calling suites: adapter contract and fake tool-round scripting in `apps/backend/tests/test_tool_adapter.py` (tools/history passthrough, inputSchema→parameters mapping, tolerant `tool_calls` parsing, marker scripting incl. `TOOL_INJECT_NAME` adversarial hook); bounded-loop primitive in `apps/backend/tests/test_tool_loop.py` (happy-path accounting, every refusal class with corrective continuation, mid-loop failure, round-cap and run-cap bounds, final-JSON-wins with single-event ledger); planning binding in `apps/backend/tests/test_planning_tool_binding.py` (model_driven self-requested rounds with no payload pre-injection and unchanged contract/citations, direct-answer honesty, cap-exhaustion fallback with disclosure, orchestration-mode parity); discipline in `apps/backend/tests/test_tool_discipline.py` (system-prompt purity, data-only tool-role framing, source-planted names refused end to end, hostile tool output inert, per-round trace contract); F009 integration in `test_technical_evaluation.py` (`tool_mode` signature refusing cross-mode comparison, `fault:tool_loop` scenario with blocking `C-TOOL-1` covering cap/unknown/malformed/mid-loop-failure, tool-loop-aware ledger accounting); web evidence rendering in `apps/web/__tests__/evidence-panel.test.tsx` (four event-type labels + collapsed-row chips + payload expansion); browser journey `apps/web/e2e/tool-loop-journey.spec.ts` behind `E2E_TOOL_LOOP=1` (deterministic stack, serial worker, keyboard + 420px spot). The owner-authorized live probe (D6 response-format mode) and TS-021 live self-requested-round evidence execute at delivery with their own evidence file, never as a CI gate. Environment note: the local machine's :8000/:3000 are the deployed containers — run E2E against a dedicated fake-API instance with CORS extended to the dev web origin (review.md L-1).
- [RESOLVED, 2026-09-04] F016 specialist-stage suites: cap formula + fake stage scripting in `apps/backend/tests/test_specialist_stages.py` (formula floors/values, analysis/design/review/revise markers incl. eval-fault modes); the source-analysis slice in `apps/backend/tests/test_source_analysis.py` (TS-001..005/015/024: contract, failure/one-in-flight/stale takeover, injection discipline with labeled payloads, disclosure states and budget, deletion completeness, run-counter isolation, deploy-backfill idempotency); the designer slice in `apps/backend/tests/test_designer_stage.py` (stage order + per-stage attribution + accounting, corrective retry then honest stage failure with preserved lessons, budgeted memory, inert design content); the reviewer slices in `apps/web/../backend/tests/test_review_stage.py` (clean pass, minor-only disclosure, revise-then-pass with round accounting and intent binding, failed-after-revise naming the review stage with no render, deck/exercise review with unchanged writer contracts and reviewer memory, unparseable review output never silently passing, structural validation never bypassed); F009 integration in `test_technical_evaluation.py` (`stage_set` + pinned `source_analysis_state` refusing divergent comparison, blocking `C-STAGE-1`, `fault:design_invalid`/`fault:review_fail` scenarios with honest evidence); web rendering in `apps/web/__tests__/specialist-stage-surfaces.test.tsx` (stage labels + review-round chips, sources analysis region with gated retry and cost line, read-only findings/design regions with severity and round captions, narration sentences); browser journey `apps/web/e2e/specialist-stage-journey.spec.ts` behind `E2E_SPECIALIST_STAGES=1` (deterministic stack: analysis badge → severe-then-revise generation → read-only regions → evidence chips → keyboard + 420px). The owner-authorized live evidence (real DeepSeek journey + full F009 re-baseline) executes at delivery, never as a CI gate. Environment note: run the deterministic E2E stack on free ports (e.g. API 8010 / web 3100) with `LESSONCANVAS_CORS_ALLOWED_ORIGINS` extended to the web origin; backend pytest needs the deploy.env credential overrides against the isolated `lessoncanvas_test` database while `apps/backend/.env` remains stale against the deployed stack.

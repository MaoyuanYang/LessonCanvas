...
- [UNKNOWN, NON_BLOCKING] The identity provider's account-deletion callback is resolved for F001 (Clerk user-deletion API), and local MinIO deletion is synchronous; hosted object-store deletion guarantees remain open. F011 verified the deletion contract on the local reference stack and documented the provider constraint set; provider selection lands with F012.

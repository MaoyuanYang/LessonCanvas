...
- `Artifact Production -> MCP tool definitions`: register generation and validation tools once and consume the same definitions from the workflow. F015: render/validate document tools stay orchestration-called deterministic post-steps — the model-driven tool loop binds grounding tools only. F016: the specialist stage composition is plans design→write→review (decks/exercises write→review) with one severity-gated revise round (`artifact_production/design.py` + `review.py` shared by the three graphs); every stage call reserves a per-run slot, writes its own trace event, and review never precedes rendering past the deterministic structural gate.

...
- [RESOLVED, 2026-09-06 correction of the 2026-08-24 F001 record] Frontend libraries (`YMY / Project Owner`): TanStack Query (server state), Tailwind CSS with a semantic token layer (styling), Radix UI (unstyled accessible primitives), lucide-react (icons). The 2026-08-24 record also named React Hook Form + Zod for forms/validation; what actually shipped in F001 and remains in use is hand-rolled controlled React forms with API-side contract validation — neither library is a dependency (`apps/web/package.json`). Correction recorded by F017 after the 2026-09-06 verification pass found the drift.

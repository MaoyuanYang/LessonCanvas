...
- Retained artifact row (`apps/web/components/artifact-run.tsx` `RetainedList`, F007 2026-08-31): shared variant of the artifact progress surfaces presenting unaffected lessons under a version transition — 沿用 status with prior-version provenance line and download slot, no resume action; consumed by all three family panels with the `noun` parameter.
- Evidence disclosure rows (`apps/web/components/evidence-panel.tsx`, F006 2026-08-31): the first concrete consumption of the Disclosure/evidence-panel and trace-density List contracts — collapsed metric rows (kind, lesson scope, latency, tokens, estimated cost, model) expanding to inert pre-wrap payload text with owner-only copy. Feature-local composition; promotion to a shared variant is deferred until F008's findings views need the same shape.
- Citation chip (`apps/web/components/citation-chip.tsx`, F014 2026-09-03): promoted from the blueprint-panel feature-local badge into one shared component with two documented variants — source-chunk citations (button chip `来源：文件名 · 第N段` expanding inline to filename, position, server-delivered excerpt, and hash prefix; `aria-expanded`, no dialogs, no motion) and standards citations (static). Consumed by the blueprint panel's objectives and, via the shared artifact progress list, by all three artifact-family panels; the evidence retrieval rows reuse the same chip visual for their 命中/排除/预算 summaries.
- Printable report route (`apps/web/app/(authed)/projects/[projectId]/report/`, F008 2026-09-01): the project's first printable surface — a dedicated route with semantic report markup and a scoped print stylesheet (`@media print` hides app chrome; `no-print` utility for screen-only guidance). Reused for the current alignment report and export-time snapshots; any future printable surface must compose the same pattern instead of a Feature-local print style.

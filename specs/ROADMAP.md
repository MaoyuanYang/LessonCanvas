...
### Current: F014 DONE — Phase 2 first feature delivered

(See the F014 entries below; the current handoff detail is recorded in the F014 block.)

### Previous: F014 NEXT — spec refined, awaiting SPEC READY approval
...
- Delivery evidence 2026-09-03 (owner-authorized: commit/push/PR, Issue update, deploy re-verification, live re-baseline): deployed chain PASS (migrations + idempotent backfill + smoke; weights baked via HF mirror with xet disabled; sample re-seeded with real embeddings); delivery-found defect M-4 fixed with regression test (deployed worker had never registered `lessoncanvas.parse_source` — latent since F001); TS-026 live re-baseline all six passes `completed/pass` under `retrieval_mode=fastembed` (better than the truncation-era baseline, which had one C-ART-1 failure). Evidence in `specs/F014-semantic-source-retrieval/live-evidence.json`.
- Delivery & DONE: PR [#29](https://github.com/MaoyuanYang/LessonCanvas/pull/29) merged as `fbe78b4` 2026-09-03 (full delivery flow authorized); main re-verified (backend 548 passed + 4 skipped + ruff clean; web 113/113 + tsc clean + eslint 0 errors). Issue #28 closed with checklist complete. `DONE: PASS` + `Roadmap Status: DONE` recorded 2026-09-03; evidence manifest in `specs/F014-semantic-source-retrieval/spec.md` Gate Record.
- Next actionable: F015 Governed Model Tool Calling (P1, `DRAFT`) or F016 Specialist Role Expansion (P1, `DRAFT`, depends on F014 DONE which is now complete) — bind an Issue under separate authorization and run `feature-dev` SPEC_REFINEMENT.

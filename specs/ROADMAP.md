...
| `F011` | Public Multi-Account Guardrails | Verify the complete system's privacy, abuse, cost, and deletion controls | Makes public use bounded and defensible | `P0` | `F009` | `REVIEW` | System-wide isolation, quotas, rate/concurrency limits, injection defense, authorized objects, operator audit, and deletion |
...
- Gates: `SPEC READY: PASS` (`d27deee5bfc8`) approved by `YMY / Project Owner` on 2026-09-01 (interactive question-form confirmation of D2 relaxed limit set — API 240/min general + 120/min expensive writes, 2 concurrent generation runs, 6 SSE streams, 200 MB/day uploads — D3 no-operator-role + account-page disclosure, D4(b) content-free retained security ledger after account deletion, D6 inclusion of the F006 M-2 worker fast-fail; D1/D5/D7–D11 evidence-resolved and confirmed with Spec approval). `UI READY: PASS` (`ux-ui-f011-r1` / `ab827a69abd6`, owner-ratified 2026-09-01). `TEST DESIGN READY: PASS` (`test-design-f011-r1` / `d4ceb6eb0d30`, TS-001..TS-019) approved 2026-09-01. Plan `plan-f011-r1` @ `850c40e8e41a` (T0–T12) valid; `Roadmap Status: READY` then `IN_PROGRESS` recorded 2026-09-01 on `feature/F011-public-multi-account-guardrails`.
- Implementation T0–T12 complete 2026-09-01: PostgreSQL-authoritative nested rate windows + admission (409 RUN_ADMISSION with active-run pointer) + SSE cap + daily upload volume; upload hardening (magic-byte sniffing, oversize-before-buffer, zip-bomb/pdf guards, filename policy); count-quota and workspace-resolution race fixes; deletion completeness (LangGraph checkpoint cascade, metadata-only residual repair ledger, visible `delete_failed` source state) + worker fast-fail (F006 M-2); download audits + `GET /account/usage` + `GET /account/audit` + D4(b) retained ledger; governed adversarial corpus + 71-path cross-account sweep + 5-workspace bounded journey; web account sections (usage/disclosure/audit) + named limit feedback; dependency audits clean (uv 0; pnpm 0 via workspace overrides). Verification: backend 454 passed + ruff clean; web 83/83 + eslint 0 errors + tsc/build clean. Implementation-found defects fixed with tests: content_type width (latent F001), workspace race, unrepairable deletion orphan (IF-1..IF-3 in review.md); TS-018 E2E env-gated with substitute coverage (M-1).
- `Roadmap Status: REVIEW` 2026-09-01; delivery (commit/push/PR/merge, Issue #22 update) pending separate authorization.
- Routed residuals confirmed in scope: F006 M-2 / F004 M-2 worker fast-fail; F001 source-count quota atomicity; F001 TQ-003 student-data false negatives; F003 D3 cost-bound assumption; F006 D9 operator disclosure; F001/F002 deferred load verification; F001 D3/D11 hosted-store constraint set (selection lands with F012).
- Next: UI branch (`UI Impact: YES`) toward `UI READY`.

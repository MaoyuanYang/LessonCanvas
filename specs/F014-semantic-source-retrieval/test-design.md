...
- TS-026: GREEN (2026-09-03, owner-authorized) — full live re-baseline on the deployed stack under `retrieval_mode=fastembed`: cultural-heritage p1/p2, natural-disasters p1/p2, travelling-around p1/p2 all `completed/pass`, zero blocking failures; retrieval-quality samples recorded per pass (`live-evidence.json`). The first attempt exposed delivery defect M-4 (unregistered worker parse task) — fixed with a regression test; the polluted attempt's harness workspaces were purged via the app deletion cascade before the clean run.

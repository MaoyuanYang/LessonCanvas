...
export function GenerationPanel({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    const active = runId && status && !TERMINAL_STATUSES.has(status);
    if (!active) {
      abortRef.current?.abort();
      return;
    }
  ...
  return (
    <div>
      ...
      {!connected && !TERMINAL_STATUSES.has(snapshot.status) ? (
        <Alert tone="warning">
      ...
      {snapshot.status === "complete" ? (
        <Alert tone="info">
          全部 {snapshot.total_count} 课教案已生成并通过结构校验，可逐课下载。
        </Alert>
      ) : null}
      ...
      {snapshot.status === "capped_failure" ? (
        <Alert tone="warning">
          已达本任务模型调用上限（{snapshot.model_call_cap} 次）。已完成教案仍可下载；可恢复剩余课程，或修订确认意图后重新生成。
        </Alert>
      ) : null}
      {snapshot.status === "partial_failure" ? (
        <Alert tone="error">
          部分课程失败：{failedLessons.map((a) => `第 ${a.lesson_index} 课`).join("、")}。已完成教案保持可用。
        </Alert>
      ) : null}
      {snapshot.status === "terminal_failure" ? (
        <Alert tone="error">生成失败且不可自动恢复。已完成教案保持可用。</Alert>
      ) : null}
      {error ? <Alert tone="error">{error}</Alert> : null}
        ...
        >
          <ul className="space-y-1">
            {narrationLines.map((line, index) => (
              <li key={`${index}-${line}`} className={index === narrationLines.length - 1 ? "" : "text-ink-secondary"}>
                {line}
              </li>
            ))}
          </ul>
      ...
      <section aria-label="课程进度" className="mb-4">
        <h3 className="mb-2 text-base font-medium">
          课程进度（{snapshot.complete_count}/{snapshot.total_count}）
        </h3>
        <ul className="space-y-2">
          {snapshot.artifacts.map((artifact: GenerationArtifact) => (
            <li
              key={artifact.id}
            ...
            >
              <div>
                ...
                <span className="ml-3 text-sm text-ink-secondary">
                  {LESSON_STATUS_LABELS[artifact.status] ?? artifact.status}
                </span>
